...
def calc_reward_avg(routes_complete):
    ...
    for src in range(1, len(routes_complete)+1):
        for dst in range(1, len(routes_complete[src])+2):
            if src != dst:
                ...
                reward += calc_reward(routes_complete[src][dst], data, src, dst)
                bwd_tmp, delay_tmp, loss_tmp = calc_avg_stats_path(routes_complete[src][dst], data)
                bwd += bwd_tmp
                delay += delay_tmp
                loss += loss_tmp
                    #print("Completato episodio ", episode +1, " per ", src, " ", dst)
    print("REWARD = ", round(float(reward/num_paths), 2), "\n\n" )
    ...
    file.writerow([0, round(float(reward/num_paths),2), round(float(bwd/num_paths),2),
